...
export default class Enemy11 extends Enemy1V1 {
	height: any;
	...
	constructor(scene: Phaser.Scene, plugin: SpinePlugin, x: number, y: number, dataKey?: string, atlasKey?: string, skin?: string, boundsProvider?: SpineGameObjectBoundsProvider, xargs?: any) {
		super(scene, plugin, x ?? 0, y ?? 0, dataKey ?? "Enemy11", atlasKey ?? "Enemy11-atlas", boundsProvider ?? new SkinsAndAnimationBoundsProvider(null, ["default"]));
	...
	deployLaser() {
        ...
        const laser = this.scene.add.sprite(laserX, laserY, "EnemyLaserTexture2");
